...
public class MoveToBallAgent : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        transform.position = new Vector3(-1.85f, 0.52f, 1.48f);
    }
    public override void CollectObservations(VectorSensor sensor)
    {
        // Example: sensor.AddObservation(transform.position); // Add the agent's position as an observation
        sensor.AddObservation(transform.position); // Add the agent's position as an observation
        sensor.AddObservation(ballTransform.position); // Add the ball's position as an observation
    }
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        float moveSpeed = 5f;
        transform.position += new Vector3(moveX, 0, moveZ) * Time.deltaTime * moveSpeed; // Move the agent based on the actions
    }
